...
export default function SealedDetail({ productId, products, addToCart, setSelectedProductId, setActivePage, setFilters, alert, user, onOpenLogin }) {
  ...
  useEffect(() => {
    // Reset view state and scroll to top when switching products
  ...
  const hasValidPhoto = (p) => {
    if (!p) return false;
    const img = p.image || p.image_url || p.img;
    return typeof img === 'string' && img.trim() !== '' && img.trim() !== 'null' && img.trim() !== 'undefined';
  };

  const similarSealed = useMemo(() => {
    if (!products || !product) return [];
    return products
      .filter(p => p && p.type === 'sealed' && p.id !== product.id && hasValidPhoto(p))
      .map(p => {
        let score = 0;
  ...
  const relatedSealed = useMemo(() => {
    if (!products || !product) return [];
    return products
      .filter(p => p && p.id !== product.id && hasValidPhoto(p))
      .map(p => {
        let score = 0;
          ...
          {product.lang && (
            <>
              ...
              <span
                className="breadcrumb-item"
                onClick={() => {
                  const catId = product.lang === 'JP' ? 'jpn' : product.lang === 'CN' ? 'chn' : 'ENG';
                  const langCode = product.lang === 'JP' ? 'JP' : product.lang === 'CN' ? 'CN' : 'ENG';
                  if (setFilters) setFilters({ game: product.game, category_id: catId, lang: langCode });
                  setActivePage('sealed-catalog');
                }}
              >
