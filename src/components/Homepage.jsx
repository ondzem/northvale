      ...
      if (mapped.length > 0) {
        ...
        try {
          const cleanMapped = mapped.map(s => ({
            ...s,
            desktopImage: (s.desktopImage && s.desktopImage.startsWith('data:')) ? '' : s.desktopImage,
            mobileImage: (s.mobileImage && s.mobileImage.startsWith('data:')) ? '' : s.mobileImage
          }));
          localStorage.setItem('northvale-cached-slides', JSON.stringify(cleanMapped));
        } catch {}
      } else {
        setSlides([]);
      }
    ...
    <div style={{ ...styles.container, paddingTop: isMobile ? '12px' : '24px', paddingBottom: isMobile ? '40px' : '96px', gap: isMobile ? '48px' : '88px' }} className="fade-in">
      ...
      <section id="popular-categories" className="category-section container">
        ...
        <div className="category-tiles-grid">
          ...
          <a href="/sealed-catalog/?game=Ostatn%C3%AD TCG" style={{ ...styles.categoryTile, textDecoration: 'none', color: 'inherit' }} className="glass-card" onClick={(e) => { if (e.button === 0 && !e.ctrlKey && !e.metaKey && !e.shiftKey && !e.altKey) { e.preventDefault(); setFilters({ game: 'Ostatní TCG' }); setActivePage('sealed-catalog'); } }}>
