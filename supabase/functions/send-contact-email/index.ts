...
serve(async (req) => {
  ...
  try {
    ...
    if (isHoneypotFilled(raw)) {
      console.warn("[send-contact-email] honeypot vyplněn — odmítnuto.");
      return new Response(JSON.stringify({ error: "Zprávu se nepodařilo ověřit." }), {
        status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" }
      });
    }
    {
      const turnstile = await verifyTurnstile((raw as any)?.turnstileToken, callerIp(req));
      if (!shouldAllow(turnstile, "send-contact-email")) {
